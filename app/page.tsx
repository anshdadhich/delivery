...
export default function Home() {
  ...
  async function fetchStats() {
    try {
      const res = await fetch(
        `/api/shipments/stats?search=${search}&severity=${severity}`
      );
      if (!res.ok) {
    ...
    <main className="p-6 max-w-7xl mx-auto space-y-8 bg-gray-50 min-h-screen text-gray-900">
      <header>
        ...
        <p className="text-gray-500">
          Upload shipments → monitor delays & severity
        </p>
      </header>
      ...
      <Card>
        ...
        <CardContent>
          ...
          >
            <label
              htmlFor="fileInput"
              className="flex-1 cursor-pointer px-4 py-2 border rounded-2xl text-gray-600 hover:bg-gray-100 hover:border-black-400 flex items-center justify-center gap-2 transition"
            >
...
{shipments.length > 0 ? (
  ...
  <p
    className={`px-2 py-1 rounded-md font-medium ${
      shipment.EcomStatus?.toLowerCase().includes("delayed") || shipment.EcomStatus?.toLowerCase().includes("cancelled")
...
{selectedShipment && (
  ...
  >
    <DialogContent
      className="
        ...
        max-w-4xl
        max-h-[90vh]
        rounded-2xl       /* outer rounding */
        border            /* border applied to dialog itself */
        shadow-2xl
    ...
    >
      <div
        className="
          ...
          overflow-x-hidden
          p-6
          bg-white         /* keep content clean white */
        "
      >
        ...
        <div className="divide-y">
          {Object.entries(selectedShipment).map(([key, value]) => {
            ...
            const isLong =
              typeof displayValue === "string" && displayValue.length > 80;
            ...
            if (isStatusField && typeof displayValue === "string") {
              const valueLower = displayValue
                .toLowerCase()
                .replace(/\s|-/g, "");

              if (
                valueLower.includes("delayed") ||
        ...
        <div className="overflow-auto border rounded-lg">
          <table className="w-full text-sm">
            <thead className="bg-gray-100">
              <tr>
                {Object.keys(shipments[0] || {}).map((key) => (
                  <th
                    key={key}
            ...
            <tbody>
              {shipments.map((row, idx) => (
                ...
                >
                  {Object.keys(shipments[0] || {}).map((field) => {
                    const value =
                      row[field] === null ||
                      row[field] === undefined ||
                      row[field] === ""
  ...
  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
    {shipments.map((row, idx) => (
      <div
        key={idx}
        className="bg-white rounded-xl border p-4 shadow hover:shadow-md transition cursor-pointer"
      >
          ...
          return (
            <div key={field} className="mb-2">
              ...
              <TooltipProvider>
                <Tooltip>
                  <TooltipTrigger asChild>
                    <p className="truncate cursor-help text-gray-800">
                      {value}
                    </p>
                  </TooltipTrigger>
                  <TooltipContent>
